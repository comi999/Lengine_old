...
public:
	...
	//static void DrawLine( Segment a_Line, Colour a_Colour )
	//{
	...
	static void DrawTriangle( Vector2 a_P0, Vector2 a_P1, Vector2 a_P2, Colour a_Colour )
	{
		...
		if ( M.x <= a_P1.x )
		{
			...
			if ( a_P0.y != a_P1.y )
			{
				...
				YMax = Math::Min( a_P0.y, -1.0f + ConsoleWindow::GetCurrentContext()->GetHeight() );

				for ( int y = YMin; y < YMax; ++y )
				{
					XLeft  = Math::Max( 0.0f, ( y - a_P0.y ) * InvGrad + a_P0.x );
					XRight = Math::Min( -1.0f + ConsoleWindow::GetCurrentContext()->GetWidth(), ( y - a_P0.y ) * InvM    + a_P0.x );
					ConsoleWindow::GetCurrentContext()->GetScreenBuffer().SetColours( Vector< short, 2 >( XLeft, y ), a_Colour, XRight - XLeft );
				}
			}
			...
			if ( a_P1.y != a_P2.y )
			{
				...
				YMax = Math::Min( a_P1.y, -1.0f + ConsoleWindow::GetCurrentContext()->GetHeight() );

				for ( int y = YMin; y < YMax; ++y )
				{
					XLeft  = Math::Max( 0.0f, ( y - a_P2.y ) * InvGrad + a_P2.x );
					XRight = Math::Min( -1.0f + ConsoleWindow::GetCurrentContext()->GetWidth(), ( y - a_P2.y ) * InvM    + a_P2.x );
					ConsoleWindow::GetCurrentContext()->GetScreenBuffer().SetColours( Vector< short, 2 >( XLeft, y ), a_Colour, XRight - XLeft );
				}
			}
		}
		...
		if ( M.x > a_P1.x )
		{
			...
			if ( a_P0.y != a_P1.y )
			{
				...
				for ( int y = YMin; y < YMax; ++y )
				{
					...
					XRight = Math::Min( -1.0f + ConsoleWindow::GetCurrentContext()->GetWidth(), ( y - a_P0.y ) * InvGrad + a_P0.x );
					ConsoleWindow::GetCurrentContext()->GetScreenBuffer().SetColours( Vector< short, 2 >( XLeft, y ), a_Colour, XRight - XLeft );
				}
			}
			...
			if ( a_P1.y != a_P2.y )
			{
				...
				for ( int y = YMin; y < YMax; ++y )
				{
					...
					XRight = Math::Min( -1.0f + ConsoleWindow::GetCurrentContext()->GetWidth(), ( y - a_P2.y ) * InvGrad + a_P2.x );
					ConsoleWindow::GetCurrentContext()->GetScreenBuffer().SetColours( Vector< short, 2 >( XLeft, y ), a_Colour, XRight - XLeft );
				}
			}
		}
